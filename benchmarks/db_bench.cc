...
static const char* FLAGS_report_file = "/home/zytian/calm_comparison/merge/report.csv";
...
class ReporterAgent {
 ...
 private:
  ...
  bool stop_;
  uint64_t kMicrosInSecond = 1000000;
};
...
class Stats {
 ...
 public:
  ...
  void FinishedSingleOp() {
    if (reporter_agent_) {
      reporter_agent_->ReportFinishedOps(1); //to do
    }
    if (FLAGS_histogram) {
...
class Benchmark {
  ...
  void PrintHeader() {
    ...
    std::fprintf(
        ...
        (((kKeySize + FLAGS_value_size * FLAGS_compression_ratio) * num_) /
         1048576.0));
    std::fprintf(stdout, "Block Cache Size: %.1ld MB\n", FLAGS_cache_size / 1024 / 1024);
    PrintWarnings();
    std::fprintf(stdout, "------------------------------------------------\n");
  }
 ...
 public:
  Benchmark()
      : cache_(FLAGS_cache_size >= 0 ? NewDualCache(FLAGS_cache_size, BytewiseComparator(), FLAGS_cache_monitor) : nullptr),
      ...
      if (method != nullptr) {
        ...
        /*if(FLAGS_ycsb){
          for (const auto& histogram : ycsb_histogram_) {
            ...
            fprintf(stdout, "%s", histogram.second.GetInfo().c_str());
            fflush(stdout);
          }
        }*/
      }
 ...
 private:
  ...
  void RunBenchmark(int n, Slice name,
    ...
    if (FLAGS_report_interval_seconds > 0) {
      reporter_agent.reset(new ReporterAgent(g_env, FLAGS_report_file,
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (leveldb::Slice(argv[i]).starts_with("--benchmarks=")) {
      ...
      FLAGS_key_prefix = n;
    } else if (sscanf(argv[i], "--cache_size=%lu%c", &long_n, &junk) == 1) {
      FLAGS_cache_size = long_n;
