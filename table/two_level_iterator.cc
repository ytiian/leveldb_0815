...
void TwoLevelIterator::InitDataBlock() {
  if (!index_iter_.Valid()) {
    ...
    if (data_iter_.iter() != nullptr &&
      ...
      Iterator* iter = (*block_function_)(arg_, options_, handle, file_number_, which_, CallerType::kCallerTypeUnknown);
...
Iterator* NewTwoLevelIterator(Iterator* index_iter,
                              ...
                              const ReadOptions& options, const uint64_t& file_number,
                              const bool& which) {
  return new TwoLevelIterator(index_iter, block_function, arg, options, file_number, which);
}
